...
public class RestControllerBoleto {
    ...
    @Autowired
    public RestControllerBoleto(BoletoService boletoService) {
    ...
    public ResponseEntity<?> listarBoletos() {
        logger.info("=== PETICIÓN LISTAR BOLETOS RECIBIDA ===");

        try {
            List<Boletos> boletos = boletoService.readAll();
            logger.info("Devolviendo {} boletos", boletos.size());
            return new ResponseEntity<>(boletos, HttpStatus.OK);
        } catch (Exception e) {
            logger.error("ERROR al listar boletos: ", e);
